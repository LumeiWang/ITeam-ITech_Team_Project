...
from rango.models import Category, Page, Comment, News, UserProfile, Likepage
...
def show_comment(request, category_name_slug, title):
    ...
    try:
        ...
        comments = Comment.objects.filter(page=page.title)
        ...
        comments = Comment.objects.filter(news=page.title)
        context_dict['page'] = page
        context_dict['comments'] = comments
        context_dict['category'] = category
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if page:
                ...
                if count == 0:
                    comment.page = page.title
                else :
                    comment.news = page.title
...
def delete(request, data):
    ...
    return redirect('/rango/myaccount/')
